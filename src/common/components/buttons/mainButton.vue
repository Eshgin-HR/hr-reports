<template>
  <button
    ...
    class="relative w-full font-medium transition-all overflow-hidden h-[54px]"
    ...
    :aria-label="text"
  >
    <span v-if="!loading" class="text-sm">{{ text }}</span>
...
export default {
  ...
  computed: {
    buttonClass() {
      ...
      if (this.type === "main") {
        buttonClassString.push("bg-white text-activeButton border border-borderGray hover:bg-activeButton hover:text-white");
